...
class ExifHeader:
    ...
    def dump_ifd(self, ifd, ifd_name, tag_dict=EXIF_TAGS, relative=0, stop_tag=DEFAULT_STOP_TAG, make=None):
        """
            ...
            if not (not self.detailed and tag in IGNORE_TAGS):
                ...
                if count * type_length > 4:
                    ...
                    if relative:
                        ...
                        if 'DxO' in make: # Handle the case of DxO One's EXIF
                            offset = tmp_offset + ifd - 12 # ifd is the mekernote section ofset
                        else:
                        ...
                        try:
                            ...
                            if isinstance(values, bytes):
                                try:
                                    ...
                                    print "Possibly corrupted field %s in %s IFD", tag_name, ifd_name
                                    logger.warning("Possibly corrupted field %s in %s IFD", tag_name, ifd_name)
                        except OverflowError:
                            print 'OverflowError at position: %s, length: %s', file_position, count
                            logger.warn('OverflowError at position: %s, length: %s', file_position, count)
                            values = ''
                        except MemoryError:
                            print 'MemoryError at position: %s, length: %s', file_position, count
    ...
    def decode_maker_note(self, verbosity=None):
        """
        ...
        # DxO
        if 'DxO' in make and note.values[0:8] == [0x44, 0x58, 0x4F, 0x20, 0x4F, 0x4E, 0x45, 0x00]:
            # The whole exif maker is 'DxO' and the MakerNote sections starts with the 'DXO\x20ONE\x0' string
            logger.debug("DxO One MakerNote")
            if verbosity is not None and 'MakerNote' in verbosity:
                print 'DxO One MakerNote'
                print '---------'
                s = ''
                ss = ''
                c = 0
                for i in note.values:
                    s += '%02X ' % i
                    ss += chr(i) if (i >= 0x20 and i <= 0x7E) else '.'
                    c = (c+1) % 32
                    if c == 0:
                        s += '| %s\n' % ss
                        ss = ''
                if not ss == '':
                    s += '%s| %s' % (' '*3*(32-len(ss)), ss)
                print s
                print '---------'

            self.dump_ifd(note.field_offset + 12, 'MakerNote', tag_dict=makernote.dxo.TAGS, relative=1, make=make)
